...
class Api:
    ...
    def save_file(self, filename, file_data_base64):
        try:
            ...
            subprocess.run(["python", "-m", "data_cleaning_pipeline.main"], cwd=project_root, check=True)
            subprocess.run(["python", "-m", "hallucination_reduction.main"], cwd=project_root, check=True)
