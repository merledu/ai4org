...
def evaluate_old_vs_new_generator(old_gen, new_gen, tokenizer, retriever, qa_pairs, fact_disc, fact_tok, device=DEVICE):
    ...
    if isinstance(old_gen, torch.nn.DataParallel):
        old_gen = old_gen.module
    if isinstance(new_gen, torch.nn.DataParallel):
        new_gen = new_gen.module
...
def evaluate_classifier(cls, tokenizer, texts, labels, device=DEVICE):
    if isinstance(cls, torch.nn.DataParallel):
        cls = cls.module
