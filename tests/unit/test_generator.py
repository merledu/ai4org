...
class TestLoadGenerator(unittest.TestCase):
    ...
    def test_load_generator_with_pad_token(
    ...
    @patch("hallucination_reduction.generator.AutoModelForCausalLM")
    @patch("hallucination_reduction.generator.AutoTokenizer")
    def test_load_generator_without_pad_token(
        self, mock_tokenizer_class, mock_model_class
    ):
...
class TestGenerateAnswer(unittest.TestCase):
    ...
    def test_generate_answer_single_sequence(self):
        ...
        prompt = "Test prompt"
        texts = generate_answer(
            mock_generator, mock_tokenizer, prompt, num_return_sequences=1
        )
    ...
    def test_generate_answer_multiple_sequences(self):
        ...
        prompt = "Test prompt"
        texts = generate_answer(
            mock_generator, mock_tokenizer, prompt, num_return_sequences=2
        )
...
class TestSFTFinetuneGenerator(unittest.TestCase):
    ...
    def test_sft_finetune_optimizer_steps(self, mock_adamw, mock_dataloader_class):
        ...
        sft_finetune_generator(
            self.mock_generator, self.mock_tokenizer, [self.qa_pairs[0]], epochs=1
        )
    ...
    def test_sft_finetune_loss_logging(
        self, mock_print, mock_adamw, mock_dataloader_class
    ):
        ...
        sft_finetune_generator(
            self.mock_generator, self.mock_tokenizer, [self.qa_pairs[0]], epochs=2
        )
