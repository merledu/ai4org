...
class TestLoadCorpus(unittest.TestCase):
    ...
    @patch('builtins.open', new_callable=mock_open, read_data="Document 1\nDocument 2\nDocument 3\n")
    ...
    @patch('builtins.open', new_callable=mock_open, read_data="Doc 1\n\n\nDoc 2\n  \nDoc 3")
    @patch('os.path.exists')
...
class TestBuildEmbeddings(unittest.TestCase):
    ...
    def test_build_embeddings(self, mock_sentence_transformer_class):
        ...
        embedder, embeddings = build_embeddings(docs, embed_model_name="test-model", device="cpu")
        
        mock_sentence_transformer_class.assert_called_once_with("test-model", device="cpu")
...
class TestRetrieveRelevantChunks(unittest.TestCase):
    ...
    def test_retrieve_relevant_chunks(self):
        ...
        mock_embedder.encode.assert_called_once_with(["test query"], convert_to_numpy=True)
...
class TestLoadModel(unittest.TestCase):
    ...
    def test_load_model_with_finetuned_weights(self, mock_cuda, mock_exists, mock_torch_load, 
    ...
    @patch('hallucination_reduction.inference.AutoTokenizer')
    @patch('torch.cuda.is_available')
    def test_load_model_without_finetuned_weights(self, mock_cuda, mock_tokenizer_class, 
                                                   mock_model_class, mock_find_best):
    ...
    @patch('hallucination_reduction.inference.AutoTokenizer')
    @patch('torch.cuda.is_available')
    def test_load_model_cuda_available(self, mock_cuda, mock_tokenizer_class, 
                                       mock_model_class, mock_find_best):
...
class TestGenerateAnswer(unittest.TestCase):
    ...
    def test_generate_answer_basic(self, mock_print, mock_streamer_class, mock_thread_class):
        ...
        result = generate_answer(mock_model, mock_tokenizer, device, question, retrieved_docs)
    ...
    @patch('hallucination_reduction.inference.TextIteratorStreamer')
    @patch('builtins.print')
    def test_generate_answer_with_multiple_docs(self, mock_print, mock_streamer_class, 
                                                  mock_thread_class):
    ...
    @patch('hallucination_reduction.inference.TextIteratorStreamer')
    @patch('builtins.print')
    def test_generate_answer_custom_max_tokens(self, mock_print, mock_streamer_class, 
                                                 mock_thread_class):
...
class TestIntegration(unittest.TestCase):
    ...
    def test_embeddings_and_retrieval_flow(self, mock_st_class):
        ...
        # Retrieve
        top_docs = retrieve_relevant_chunks("test query", embedder, embeddings, docs, top_k=1)
