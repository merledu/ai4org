...
class TestComputeReward:
    ...
    def test_returns_tuple(self, mock_discriminators, mock_tokenizers):
        ...
        with patch(
            "hallucination_reduction.rl_utils.discriminator_predict_text"
        ) as mock_pred:
            mock_pred.return_value = [{"probs": [0.3, 0.7]}]

            with patch(
                "hallucination_reduction.rl_utils.overlap_fact_check", return_value=0.8
            ):
    ...
    def test_returns_float_reward_and_debug_dict(
        self, mock_discriminators, mock_tokenizers
    ):
        """Test return types."""
        fact_disc, style_disc, safety_disc = mock_discriminators
        fact_tok, style_tok, safety_tok = mock_tokenizers
...
class TestMonteCarloRewards:
    ...
    def test_returns_tuple_of_three(
        self, mock_generator, mock_tokenizer, mock_discriminators, mock_tokenizers
    ):
        ...
        with patch("hallucination_reduction.rl_utils.generate_answer") as mock_gen:
            ...
            with patch(
                "hallucination_reduction.rl_utils.compute_reward"
            ) as mock_reward:
    ...
    def test_returns_avg_reward_debug_samples(
        self, mock_generator, mock_tokenizer, mock_discriminators, mock_tokenizers
    ):
        """Test return types."""
        fact_disc, style_disc, safety_disc = mock_discriminators
    ...
    def test_sets_generator_to_eval_mode(
        self, mock_generator, mock_tokenizer, mock_discriminators, mock_tokenizers
    ):
    ...
    def test_computes_average_reward(
        self, mock_generator, mock_tokenizer, mock_discriminators, mock_tokenizers
    ):
...
class TestReinforceUpdate:
    ...
    def mock_generator(self):
        ...
        # Logits must require grad for backward() to work in tests
        logits = torch.randn(1, 10, 1000, requires_grad=True)
    ...
    def mock_tokenizer(self):
        ...
        mock_encoding.input_ids = torch.randint(0, 1000, (1, 10))
        mock_encoding.attention_mask = torch.ones(1, 10)
    ...
    def test_returns_float(self, mock_generator, mock_tokenizer, mock_optimizer):
        ...
        result = reinforce_update(
            mock_generator, mock_tokenizer, prompts, samples, rewards, mock_optimizer
        )

        assert isinstance(result, float)

    def test_handles_empty_prompts(
        self, mock_generator, mock_tokenizer, mock_optimizer
    ):
        """Test handling of empty prompts list."""
        result = reinforce_update(
            mock_generator, mock_tokenizer, [], [], [], mock_optimizer
        )

        assert result == 0.0

    def test_sets_generator_to_train_mode(
        self, mock_generator, mock_tokenizer, mock_optimizer
    ):
    ...
    def test_handles_generator_error_gracefully(self, mock_tokenizer, mock_optimizer):
        ...
        with patch("builtins.print"):
            result = reinforce_update(
                mock_gen, mock_tokenizer, prompts, samples, rewards, mock_optimizer
            )
    ...
    def test_returns_history(self, mock_components, mock_qa_pairs):
        ...
        with patch("hallucination_reduction.rl_utils.monte_carlo_rewards") as mock_mc:
            mock_mc.return_value = (
                0.7,
                [{"sample": "test", "debug": {"combined": 0.7}}],
                ["sample"],
            )

            with patch(
                "hallucination_reduction.rl_utils.reinforce_update", return_value=0.1
            ):
    ...
    def test_creates_optimizer(self, mock_components, mock_qa_pairs):
        ...
        with patch("hallucination_reduction.rl_utils.monte_carlo_rewards") as mock_mc:
            ...
            with patch(
                ...
                with patch(
                    "hallucination_reduction.rl_utils.optim.AdamW"
                ) as mock_adamw:
                    with patch("torch.save"):
                        with patch("builtins.print"):
                            mock_components["retriever"].retrieve.return_value = [
